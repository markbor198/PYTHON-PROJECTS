...
def groupSum5(start, nums, target):
    ...
    #elif start < len(nums) and nums[start+1] == 1 :
...
def splitArrayHelper(start,nums,group1,group2):
    ...
    if (start>= len(nums)):
        ...
        return splitArrayHelper(start +1, nums, group1 + nums[start],group2) or \
...
def splitOdd10Helper(start,nums,multiple,odd):
     if (start>= len(nums)):
        return (multiple%10==0) and odd%2 == 1
     else:
        
        
       
        
        return splitOdd10Helper(start +1, nums, multiple + nums[start],odd) or \
               splitOdd10Helper(start+1, nums, multiple, odd + nums[start])
...
def split53Helper(start,nums,mult_five,mult_three):
    if (start>= len(nums)):
        return (mult_five % 5 == 0 and mult_three % 3 ==0 )
    else:
        
        
        
        return split53Helper(start +1, nums, mult_five + nums[start],mult_three) or \
               split53Helper(start+1, nums, mult_five, mult_three + nums[start])
